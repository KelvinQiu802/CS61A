...
def is_swap(player_score, opponent_score):
    ...
    oppo_ones_digit, oppo_tens_digit = opponent_score % 10, opponent_score // 10 % 10
    return abs(player_ones_digit - oppo_ones_digit) == oppo_tens_digit
    # END PROBLEM 4
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0, score1, last_score=last_score, running_high=running_high):
        if who == 0:
            diff, last_score = score0 - last_score, score0
        else:
            diff, last_score = score1 - last_score, score1
...
def make_averaged(original_function, trials_count=1000):
    ...
    def f(*args, count=count, trials_count=trials_count):
        times = trials_count
        while trials_count:
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    number, max_score, max_number = 10, 0, 10
    while number:
        score = averaged_dice(number, dice)
        if score >= max_score:
            max_score, max_number = score, number
        number -= 1
    return max_number
    # END PROBLEM 9
...
def bacon_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    bacon = 10 - opponent_score % 10 + opponent_score // 10 % 10
