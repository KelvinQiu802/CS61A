...
class VendingMachine:
    ...
    def add_funds(self, value):
        if self.stock == 0:
            return 'Inventory empty. Restocking required. Here is your ${0}.'.format(value)
        else:
            self.funds += value
...
def is_bst(t):
    ...
    if len(t.branches) > 2:
        ...
        child = t.branches[0]
        if t.label > child.label:
            # left
            return is_bst(child) and t.label >= bst_max(child)
        else:
            # right
            return is_bst(child) and t.label < bst_min(child)
    else:
