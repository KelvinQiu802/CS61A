...
def g_iter(n):
    ...
    """
    "*** YOUR CODE HERE ***"
    first, second, third, fourth,  i = 1, 2, 3, 0, 4
    if n <= 3:
        return n
    while i <= n:
        fourth = third + 2 * second + 3 * first
...
def missing_digits(n, count=0):
    ...
    diff = last - without_last % 10
    if diff > 1:
        return missing_digits(without_last, count + diff - 1)
    else:
        return missing_digits(without_last, count)
