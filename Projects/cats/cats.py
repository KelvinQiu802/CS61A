...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def chooser(p):
        return any([t in split(remove_punctuation(lower(p))) for t in topic])
    return chooser
    # END PROBLEM 2
...
def fastest_words(game):
    ...
    for i in words:
        min_time, min_player = float('inf'), 0
        for j in players:
            t = time(game, j, i)
            if t < min_time:
                min_time, min_player = t, j
        fastest[min_player].append(all_words(game)[i])
    return fastest
    # END PROBLEM 10
